...
if(mission_system == true){
  ...
  if(authorityStatus == true) return message.reply({embeds:[embed.setDescription(`${member} <t:${(guildAutoStaff.startingdate/1000).toFixed()}:R> <@${guildAutoStaff.staffID}> tarafından yetkili yapılmıştır.`)]});
  else{
    ...
    let msg = await message.channel.send({content:`[${member}]`,embeds:[embed.setDescription(`${message.member} daveti ile yetkili olmayı kabul ediyor musun ?`)],components:[row]})
    ...
    if(i.customId == "evet"){
      ...
      message.reply({embeds:[embed.setDescription(`${message.member}, ${member} yetkili olarak işaretlendi!`)]})
